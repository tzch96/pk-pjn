...
# Laboratorium 6: rekomendacje oparte na treści
...
służącego do porównywania kolejnych wersji
...
Uwaga: na jakość rekomendacji wpływa tylko lista dzieł
rekomendowanych przez program `rekomenduj.py` i kolejność
dzieł na tej liście. **Nie należy** używać do oceny jakości
umownych liczb, które program `rekomenduj.py` wypisuje
jako miarę podobieństwa dokumentów.

    a. Sprawdzić wpływ lematyzacji wyrazów na jakość modelu.
    Skopiować swoje funkcje `formy_podstawowe()` i
    `jednoznaczna_forma_podstawowa()` z
    [laboratorium 1](https://github.com/PK-PJN/laboratorium01).
    Jeśli Morfeusz nie został zainstalowany, to skopiować również
    plik `polski.sqlite3` i fragment kodu, który go otwiera.
    Zastąpić tekst każdego artykułu przez tekst złożony
    z form podstawowych odpowiednich wyrazów.
    Stworzyć nowy model itd.

    b. Dodać do `STOP_WORDS` wybrane (z głową!)
    najczęstsze wyrazy, wypisane przez program
    `zadanie.py`, np. *kategoria*, *linki*, *zewnętrzne* itp.
    Stworzyć nowy model itd.

    c. Dotychczas usuwaliśmy liczby z tekstów artykułów.
    Sprawdzić wpływ ich nieusuwania na jakość modelu:
    usunąć fragment `0-9` z napisu przy zmiennej `NONLETTERS_RE`.
    Stworzyć nowy model itd.

    d. Dotychczas nie pozostawialiśmy argumentów
...
który odpowiada najlepszemu modelowi.
